...
def cos_sim(x1,x2):
    res=torch.sum(x1*x2) /(torch.norm(x1)*torch.norm(x2)+0.000000001)
    # print(res)
    return res

def prune_edge(x,edge_index,T):
    rows = edge_index[0]
    cols = edge_index[1]
    new_edges=[]
    for i in range(len(rows)):
        if cos_sim(x[rows[i]],x[cols[i]]) >= T:
            new_edges.append([rows[i].item(),cols[i].item()])
    print(new_edges)
    print(type(new_edges))
...
new_edges=prune_edge(data.x,train_edge_index,args.T)

train_edge_index = new_edges.to(device)
