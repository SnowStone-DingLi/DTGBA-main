...
def obtain_attach_nodes(args,node_idxs, size):#随机选择
...
def obtain_attach_nodes_by_cluster(args,y_pred,model,node_idxs,x,labels,device,size):
    ...
    for id in range(x.shape[0]):
        tmp_center_label = y_pred[id]#预测标签
        tmp_tar_label = args.target_class#目标标签
        
        tmp_center_x = cluster_centers[tmp_center_label]#预测标签对应的center
        tmp_tar_x = cluster_centers[tmp_tar_label]#目标标签对应的center

        dis = np.linalg.norm(tmp_center_x - x[id].detach().cpu().numpy())
        dis_tar = np.linalg.norm(tmp_tar_x - x[id].cpu().numpy())
        distances.append(dis)#和预测标签对应的center之间的距离
        distances_tar.append(dis_tar)#和目标标签对应的center之间的距离
    ...
    return candidate_nodes#更靠近中心，则破坏力越强
